#! /usr/bin/env python
# -*- coding: utf-8 -*-


class Node(object):  # 节点类
    # 初始化，需要传入节点的数据
    def __init__(self, data):
        self.data = data    # 数据域，用于存放数据元素
        self.next = None    # 指针域，指向下一个节点

    # 返回节点的数据
    def get_data(self):
        return self.data

    # 更新节点的数据
    def set_data(self, new_data):
        self.data = new_data

    # 返回后继节点
    def get_next(self):
        return self.next

    # 变更后继节点
    def set_next(self, new_next):
        self.next = new_next


class SingleLinkList(object):  # 链表类
    # 初始化，头结点为空
    def __init__(self):
        self.head = None

    # 添加节点，添加的新节点作为新的头结点
    def add(self, data):
        new_node = Node(data)
        new_node.next = self.head
        self.head = new_node

    # 包含查询，传入值，返回该值在链表中是否存在
    def search(self, data):
        checking = self.head  # 从头结点开始查询
        while checking != None:
            if checking.get_data() == data:  # 查找到，返回True
                return True
            checking = checking.get_next()  # 查询下一个节点
        return False  # 遍历到最后也未能找到，返回False

    # 删除节点，将第一个具有传入值的节点从链表中删除
    def remove(self, data):
        checking = self.head  # 从头结点开始查询
        previous = None  # 记录前一个节点，头结点的前一个节点为None

        while checking != None:
            if checking.get_data() == data:  # 查找到，跳出查找循环
                break
            previous = checking  # 更新前一个节点
            checking = checking.get_next()  # 查询下一个节点

        if previous == None:  # 情况一：如果头结点便是查找的节点
            self.head = checking.get_next()
        else:  # 情况二：查找的节点不在头结点，即，存在前驱节点
            previous.set_next(checking.get_next())

    # 判断链表是否为空
    def isEmpty(self):
        return self.head == None

    # 返回链表长度
    def size(self):
        count = 0
        counting = self.head  # 从头结点开始计数
        while counting != None:
            count += 1
            counting = counting.get_next()
        return count


if __name__ == '__main__':
    link_list = SingleLinkList()
    print('link_list.empty() == %s, link_list.size() == %s' % (link_list.isEmpty(), link_list.size()))

    for i in range(1, 4, 1):
        link_list.add(i)
    print('link_list.empty() == %s, link_list.size() == %s' % (link_list.isEmpty(), link_list.size()))

    print('1 is%s in link_list' % ('' if link_list.search(1) else ' not',))
    print('3 is%s in link_list' % ('' if link_list.search(3) else ' not',))
